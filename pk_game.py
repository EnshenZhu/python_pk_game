...
# player has the passive ability of regening 20% of current HP when being attacked, in the chance of 8%
...
while True:
    ...
    player_HP_full = player_HP  # save the initial full HP of the player
    ...
    enemy_HP_full = enemy_HP  # save the initial full HP of the enemy
    ...
    while player_HP and enemy_HP:
        ...
        if enemy_double_damage == 1:
            print(
                '【敌人】向【玩家】发起了攻击，造成%d伤害，【玩家】剩余血量%d'
                % (enemy_stage_AP, player_HP)
            )
            time.sleep(1)
        else:
            print(
                '【敌人】向【玩家】发起了攻击，造成%d伤害，【玩家】剩余血量%d get double damaged!!!'
                % (enemy_stage_AP, player_HP)
            )
            time.sleep(1)
        ...
        if (
                player_HP > 0
                and random.choices(population=[1, 2], weights=[92, 8])[0] == 2
        ):
            # player cast passive ability in 8% chance
            # random_result=1 stands for fail to cast, random_result=2  stands for successfully cast the passive ability

            if player_HP * 1.2 > player_HP_full:
                player_HP = player_HP_full
            else:
                player_HP = int(player_HP * 1.2)
            print('【玩家】打出被动技能，回血20%，【玩家】剩余血量' + str(player_HP))

        ####################我是华丽的分界线####################

        if player_double_damage == 1:
            print('【玩家】向【敌人】发起了攻击，造成%d伤害，【敌人】剩余血量%d'
                  % (player_stage_AP, enemy_HP)
                  )
        else:
            print('【玩家】向【敌人】发起了攻击，造成%d伤害，【敌人】剩余血量%d get double damaged!!!'
                  % (player_stage_AP, enemy_HP)
                  )

        if (
                enemy_HP > 0
                and random.choices(population=[1, 2], weights=[90, 10])[0] == 2
        ):
            # enemy cast passive ability in 10% chance
            # random_result=1 stands for fail to cast, random_result=2  stands for successfully cast the passive ability
    ...
    while question != 'Y' and question != 'N':
        question = (input('One more term? Y/N ')).upper()
        if question == 'N':
            break
